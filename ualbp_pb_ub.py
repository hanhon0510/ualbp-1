...
def solve_ualbp_with_ub(num_tasks, lb, tasks, precedences, cycle_time, timeout=900):
    ub = min(2 * lb, num_tasks)
    best_sol = None
    best_st = None
    ...
    for s in range(ub, lb - 1, -1):
        ...
        if remaining <= 0:
            ...
            return best_sol, best_st, total_time, "TLE"

        sol, final_st, elapsed = solve_instance_with_timeout(
            num_tasks, s, tasks, precedences, cycle_time, timeout=remaining
        )
    ...
    total_time = time.time() - start_time
    if best_sol is None:
        return None, None, total_time, "TLE"
    else:
        return best_sol, best_st, total_time, "OK"
...
def process_instances(csv_file, output_csv, output_txt, start_row, end_row):
    ...
    with open(csv_file, newline='') as f:
        ...
        for row_index, row in enumerate(reader, start=1):
            ...
            try:
                lb_val = row['lb'].strip()
                lb = 1 if lb_val == 'NA' else int(lb_val)
            except Exception as e:
                print(f"Error parsing lb in row {row_index}: {e}", flush=True)
                continue

            parts = full_name.rsplit('-', 1)
            if len(parts) != 2:
                print(f"Skipping row {row_index} with invalid name format: {full_name}", flush=True)
                continue
            instance_name, cyc_str = parts
            try:
                cycle_time = int(cyc_str)
            except Exception as e:
                print(f"Error parsing cycle_time in row {row_index}: {e}", flush=True)
                continue

            try:
                num_tasks, tasks, precedences_inst = read_ualbp_file(f"dataset/{instance_name}")
            except Exception as e:
                print(f"Error reading instance file for row {row_index}: {e}", flush=True)
                continue

            print(f"Row {row_index}: {instance_name}, lb={lb}, cycle_time={cycle_time}", flush=True)

            sol, final_st, elapsed_time, status = solve_ualbp_with_ub(
                num_tasks, lb, tasks, precedences_inst, cycle_time, timeout=900
            )

            if sol is None:
                print(f"Row {row_index}: no solution found within budget.", flush=True)
                results.append({
                    'name': full_name,
                    'lb': lb,
                    'ub': min(2*lb, num_tasks),
                ...
                if status == "TLE":
                    print(f"Row {row_index}: timed out after {elapsed_time:.2f}s, recording best solution with {final_st} stations.", flush=True)
                else:
                    print(f"Row {row_index}: solved in {elapsed_time:.2f}s with {final_st} stations.", flush=True)

                results.append({
                    'name': full_name,
                    'lb': lb,
                    'ub': min(2*lb, num_tasks),
                    'final_num_stations': final_st,
                    'time': f"{elapsed_time:.2f}"
                })
                write_solution_to_txt(full_name, sol, output_txt)

    with open(output_csv, 'w', newline='') as out:
        fieldnames = ['name', 'lb', 'ub', 'final_num_stations', 'time']
...
if __name__ == '__main__':
    ...
    process_instances(input_csv, output_csv, output_txt, start_row=260, end_row=260)